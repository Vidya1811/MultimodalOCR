...
def run_eval(input_path, output_path):
    ...
    for index, data_item in enumerate(tqdm(predict_file)):
        if data_item["type"] == "APP agent en" or data_item["type"] == "ASCII art classification en" or data_item["type"] == "math QA en" \
            ...
            if not isinstance(data_item["predict"], str) or len(data_item["predict"]) == 0:
                data_item["score"] = 0
                data_item["bleu"] = 0
                data_item["meteor"] = 0
                data_item["f1"] = 0
    ...
    for task_name in task_type_list:
        ...
        bleu_total, meteor_total, f1_total, edit_total = 0, 0, 0, 0
        ...
        if total_len > 0:
            ...
            if task_name == "fine-grained text recognition en" or task_name == "full-page OCR en":
                ...
                print(f"  EditDist (normalized): {edit_total / total_len:.3f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate OCRBench-style predictions.")
    ...
    run_eval(args.input_path, args.output_path)
